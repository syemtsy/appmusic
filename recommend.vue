	...
	<view>
		<template>
			<view>
					...
					<uni-list-item title="推荐歌单"></uni-list-item>
					<uni-grid :column="3">
					...
					<uni-list-item title="排行榜"></uni-list-item>
					<uni-grid :column="3">
					    <uni-grid-item v-for="(item,index) in toplist" :key="index">
							<image class="grid-image" :src="item.url" mode="aspectFit" >
							</image>
					    </uni-grid-item>
					</uni-grid>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uni.request({
				url: this.musicApi+'/personalized',
				method: 'GET',
				data: {},
				success: res => {
					for (let i = 0; i < 6; i++) {
